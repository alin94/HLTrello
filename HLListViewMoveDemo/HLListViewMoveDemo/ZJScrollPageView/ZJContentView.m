...
- (instancetype)initWithFrame:(CGRect)frame segmentView:(ZJScrollSegmentView *)segmentView parentViewController:(UIViewController *)parentViewController delegate:(id<ZJScrollPageViewDelegate>) delegate {
    self = [self initWithFrame:frame collectionViewflowlayout:nil segmentView:segmentView parentViewController:parentViewController delegate:delegate];
    return self;
}
...
- (void)layoutSubviews {
    ...
    if (self.currentChildVc) {
        self.currentChildVc.view.frame = CGRectMake(0, 0, self.collectionViewLayout.itemSize.width, self.collectionViewLayout.itemSize.height);
    }
}
...
- (void)setupChildVcForCell:(UICollectionViewCell *)cell atIndexPath:(NSIndexPath *)indexPath {
    ...
    _currentChildVc.view.frame = CGRectMake(0, 0, self.collectionViewLayout.itemSize.width, self.collectionViewLayout.itemSize.height);
...
//        collectionView.pagingEnabled = YES;
        ...
        collectionView.shouldRecognizeSimultaneously = self.segmentView.segmentStyle.shouldRecognizeSimultaneously;
...
-(void)setNoSlide:(BOOL)noSlide {
    _noSlide = noSlide;
    _collectionView.noSlide = _noSlide;
}
