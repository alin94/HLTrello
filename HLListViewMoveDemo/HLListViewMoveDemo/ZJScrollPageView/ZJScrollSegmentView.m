...
static CGFloat const contentSizeXOff = 15.0;
...
- (void)titleLabelOnClick:(UITapGestureRecognizer *)tapGes {
    ...
    BOOL canSelect = YES;
    if (self.delegate && [self.delegate respondsToSelector:@selector(segmentViewEnableSelectIndex:)]) {
        canSelect = [self.delegate segmentViewEnableSelectIndex:currentLabel.tag];
    }
    if (!canSelect) {
        return;
    }
...
- (void)setupTitles {
    ...
    for (NSString *title in self.titles) {
        ...
        titleView.tag = index;
        ;
        titleView.font = [UIFont systemFontOfSize:self.segmentStyle.titleFontSize];
...
- (void)setupScrollViewAndExtraBtn {
    ...
    //        scrollW = maxX;
    //    }
    if (self.segmentStyle.extraBtnRightMargin) {
        scrollW -= self.segmentStyle.extraBtnRightMargin;
    }
    self.scrollView.frame = CGRectMake(0.0, 0.0, scrollW, self.zj_height);

    if (self.extraBtn) {
        self.extraBtn.frame = CGRectMake(scrollW , extraBtnY, extraBtnW, self.zj_height - 2*extraBtnY - 2);
    }
}
...
- (void)setUpTitleViewsPosition {
    ...
    if (self.segmentStyle.isAutoAdjustTitlesWidth && self.segmentStyle.isScrollTitle) {
        float allTitleWidth = self.segmentStyle.firstTitleMargin;
        for (int i = 0 ; i< self.titleWidths.count; i++) {
            allTitleWidth += [self.titleWidths[i] floatValue]+self.segmentStyle.titleMargin;
        }
        if (allTitleWidth < self.scrollView.bounds.size.width - self.segmentStyle.firstTitleMargin) {
            self.segmentStyle.scrollTitle = NO;
            self.segmentStyle.aveScrollLineWidth = YES;
        }
    }
    ...
    if (!self.segmentStyle.isScrollTitle) {// 标题不能滚动, 平分宽度
        ...
        NSInteger index = 0;
        float lastLableMaxX = self.segmentStyle.firstTitleMargin;
        float addedMargin = 0.0f;
        if (self.segmentStyle.isAutoAdjustTitlesWidth) {
            
            float allTitlesWidth = self.segmentStyle.firstTitleMargin;
            for (int i = 0; i<self.titleWidths.count; i++) {
                allTitlesWidth = allTitlesWidth + [self.titleWidths[i] floatValue] + self.segmentStyle.firstTitleMargin;
            }
            
            addedMargin = (allTitlesWidth + self.segmentStyle.firstTitleMargin) < self.scrollView.bounds.size.width ? (self.scrollView.bounds.size.width - allTitlesWidth)/self.titleWidths.count : 0 ;
        }
        BOOL lastRight = self.segmentStyle.lastTitleKeepRight;
        for (ZJTitleView *titleView in self.titleViews) {
            titleW = [self.titleWidths[index] floatValue];
            if (lastRight && index==self.titleViews.count - 1) {
                titleX = self.scrollView.bounds.size.width - titleW - self.segmentStyle.firstTitleMargin;
            }else{
...
- (void)setupScrollLineAndCover {
    ...
    if (self.scrollLine) {
        if (!self.segmentStyle.isScrollTitle) {
            ...
            if (self.segmentStyle.aveScrollLineWidth) {
                coverW = self.zj_width/self.titles.count;
                coverX = 0;
            }
        }
        CGFloat w = coverW;
        CGFloat x = coverX;
        CGFloat y = self.zj_height - self.segmentStyle.scrollLineHeight - self.segmentStyle.scrollLineBottomMargin;
        if (self.segmentStyle.scrollLineWidth > 0) {
            w = self.segmentStyle.scrollLineWidth;
            x = firstLabel.zj_x + (firstLabel.zj_width-w)/2;
        }
        self.scrollLine.frame = CGRectMake(x , y , w , self.segmentStyle.scrollLineHeight);
    ...
    if (self.segmentStyle.showBottomLine) {
        UIImageView * bottomlineImageView = [[UIImageView alloc]initWithImage:[UIImage imageNamed:@"Line_cccccc_icon.png"]];
        bottomlineImageView.frame = CGRectMake(0, self.zj_height - 1, self.bounds.size.width, 1);
...
- (void)adjustUIWhenBtnOnClickWithAnimate:(BOOL)animated taped:(BOOL)taped {
    ...
    [UIView animateWithDuration:animatedTime animations:^{
        ...
        if (weakSelf.scrollLine) {
            if (weakSelf.segmentStyle.scrollLineWidth > 0) {
                weakSelf.scrollLine.zj_x = currentTitleView.zj_x + (currentTitleView.zj_width - weakSelf.scrollLine.zj_width)/2;
            }else if (weakSelf.segmentStyle.isScrollTitle) {
                ...
                if (self.segmentStyle.isAdjustCoverOrLineWidth) {
                    ...
                    if (self.segmentStyle.aveScrollLineWidth) {
                        weakSelf.scrollLine.zj_x = _currentIndex*self.zj_width/self.titles.count;
                    }else{
                        weakSelf.scrollLine.zj_x = currentTitleView.zj_x;
                        weakSelf.scrollLine.zj_width = currentTitleView.zj_width;
                    }
    ...
    if (_reloadData) {
        _reloadData = NO;
        if (self.titleFontChangeBlock) self.titleFontChangeBlock();
        return ;
    }
...
- (void)adjustUIWithProgress:(CGFloat)progress oldIndex:(NSInteger)oldIndex currentIndex:(NSInteger)currentIndex {
    ...
    if (self.scrollLine) {
        if (self.segmentStyle.scrollLineWidth > 0) {
            self.scrollLine.zj_x =  oldTitleView.zj_x + (currentTitleView.zj_width - self.scrollLine.zj_width)/2 + xDistance*progress;
        }else if (self.segmentStyle.isScrollTitle) {
            ...
            if (self.segmentStyle.isAdjustCoverOrLineWidth) {
                ...
                if (self.segmentStyle.aveScrollLineWidth) {
                    self.scrollLine.zj_x = self.scrollLine.zj_width* oldIndex + (currentIndex - oldIndex)*self.scrollLine.zj_width*progress;
...
- (void)setSelectedIndex:(NSInteger)index animated:(BOOL)animated {
    BOOL canSelect = YES;
    if (self.delegate && [self.delegate respondsToSelector:@selector(segmentViewEnableSelectIndex:)]) {
        canSelect = [self.delegate segmentViewEnableSelectIndex:index];
    }
    if (!canSelect) {
        return;
    }
...
- (void)reloadTitlesAtCurrentWithNewTitles:(NSArray *)titles {
    [self.scrollView.subviews makeObjectsPerformSelector:@selector(removeFromSuperview)];
//    _currentIndex = 0;
//    _oldIndex = 0;
    self.titleWidths = nil;
...
- (void)reloadData {
    _reloadData = YES;
    [self reloadTitlesAtCurrentWithNewTitles:_titles];
}
...
- (UIView *)scrollLine {
    ...
    if (!_scrollLine) {
        ...
        if (self.segmentStyle.scrollLineCornerRadius) {
...
{
    if (!self.segmentStyle.scrollRotateExtraBtn) {
        return;
    }
    CGFloat offsetX = scrollView.contentOffset.x;
    if (offsetX> 0) {
        CGFloat sizeW = scrollView.contentSize.width;
        CGFloat offset = sizeW - offsetX- scrollView.frame.size.width;
        if (-offset > 1) {
            CGFloat angle = offset/_extraBtn.frame.size.width*4*M_PI;
            CGFloat alpha = 1 + offset/(_extraBtn.frame.size.width*4);
            if (alpha < 0.3) {
                alpha = 0.3;
            }
//            [UIView animateWithDuration:0.2 animations:^{
                self.extraBtn.transform = CGAffineTransformMakeRotation(angle);
                self.extraBtn.alpha = alpha;
//            }];
        }else{
            self.extraBtn.transform = CGAffineTransformIdentity;
        }
    }
}
